...
def load_single_dataset(path, prefix1, prefix2, small_idx=2, small_dim=728):
    ...
    return {'train': data_train,'valid': data_valid,'test': data_test,'num_nodes_train': data_all['train'].x.shape[0],'num_nodes_all': x_all.shape[0], 'dataset_name': f"{prefix1}_{prefix2}"}
def find_all_datasets(path):
    feature_files = glob.glob(os.path.join(path, "*_*_*_features.csv"))
    dataset_patterns = {}
    for f in feature_files:
        basename = os.path.basename(f)
        match = re.match(r'(.+)_(train|valid|test)_(.+)_features\.csv', basename)
        if match:
